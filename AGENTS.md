...
- `uv run training.py --config config_dpo.yaml` runs a training job using the YAML config.
...
- For validation, do a short smoke run by temporarily setting `dataset.subset` (e.g., `train[:1%]`) and `dpo_training.epochs: 1` in `config_dpo.yaml`, then run `uv run training.py --config config_dpo.yaml`.
